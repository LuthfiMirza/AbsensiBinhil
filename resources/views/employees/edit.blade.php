...
<div class="card form-card">
    <form method="POST" action="{{ route('employees.update', $employee) }}">
        @csrf @method('PUT')
            ...
            <div class="table-header" style="padding:24px 0 12px;margin-top:8px;"><div><h3>Akun Login Petugas</h3><p>Isi password hanya jika ingin mengganti password. Jika petugas lama belum punya akun, isi email dan password untuk membuat akun.</p></div></div>
            <div class="filter-grid" style="align-items:start;">
                <div class="form-field" style="flex:1;min-width:240px;"><label>Email Login</label><input type="email" name="email" value="{{ old('email', $employee->user?->email) }}" placeholder="petugas@email.com" class="form-control">@error('email')<p class="field-error">{{ $message }}</p>@enderror</div>
                <div class="form-field" style="flex:1;min-width:220px;"><label>Password Baru</label><input type="password" name="password" class="form-control">@error('password')<p class="field-error">{{ $message }}</p>@enderror</div>
                <div class="form-field" style="flex:1;min-width:220px;"><label>Konfirmasi Password Baru</label><input type="password" name="password_confirmation" class="form-control"></div>
            </div>
